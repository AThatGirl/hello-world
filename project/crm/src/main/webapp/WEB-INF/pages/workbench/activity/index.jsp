...
<meta charset="UTF-8">
	...
	$(function(){
		...
		//当容器加载完成后，对容器调用工具函数
		// $("input[name='xxx']").datetimepicker
		$(".mydate").datetimepicker({
			language:'zh-CN',//语言
			format:'yyyy-mm-dd',//日期格式
			minView:'month',//可以选择最小的视图
			initialDate:new Date(),//初始化显示的日期
			autoclose:true,//设置显示完日期后，日历自动关闭
			todayBtn:true,//是否显示今天的按钮，默认是false
			clearBtn:true//是否增加清空按钮，默认是false
		});
		//查询所有数据的第一页以及所有数据的总条数,默认值1,10
		...
		$("#editActivityBtn").click(function () {
			...
			$.ajax({
				...
				success:function (data) {
					$("#edit-id").val(data.id);
					//将owner的id值，赋值给select,浏览器会匹配select里面的option和owner值相同的value
		...
		//给删除事件添加按钮
		$("#deleteActivityBtn").unbind('click').click(function () {
			//获取列表中所有被选中的checkbox
			let checkedIds=$("#tBody input[type='checkbox']:checked");
			if (checkedIds.size()==0){
				alert("请选择所要删除的市场活动");
		...
		$("#importActivityBtn").click(function () {
			...
			//FormData是ajax提供的接口，可以模拟键值对向后台提交参数，优势：可以提交二进制数据
	...
	function queryActivityByConditionForPage(pageNo,pageSize) {
        ...
        $.ajax({
            ...
            success:function (data){
                ...
                //显示市场活动列表
                //遍历activityList,拼接所有行数据,index:下标，obj:循环变量
                let htmlStr="";
				...
				$("#pagination").bs_pagination({
					currentPage:pageNo,//当前页号，相当于pageNo,默认1

					rowsPerPage:pageSize,//显示每页条数,相当于pageSize
					totalRows:data.totalRows,//总条数
					totalPages:totalPages,//总页数，必填参数

					visiblePageLinks: 5,//一组最多可以显示的卡片数目

					showGoToPage: true,//控制是否显示：“跳转到”部分，默认显示
					showRowsPerPage: true,//控制显示：“每页显示条数部分”
					showRowsInfo: true,//是否显示记录的信息

					//用户切换页号就会执行这个,每次返回切换后的currentPage和rowsPerPage
		...
		<div style="width: 100%; position: absolute;top: 5px; left: 10px;">
				...
				<form class="form-inline" role="form" style="position: relative;top: 8%; left: 5px;">
				  ...
				  <div class="form-group">
				    <div class="input-group">
					  ...
					  <input class="form-control" type="text" id="startData" />
				    </div>
				  </div>
				  <div class="form-group">
				    <div class="input-group">
			...
			<div class="btn-toolbar" role="toolbar" style="background-color: #F7F7F7; height: 50px; position: relative;top: 5px;">
				...
				<div class="btn-group" style="position: relative; top: 18%;">
                    ...
                    <button id="exportAllActivitiesBtn" type="button" class="btn btn-default"><span class="glyphicon glyphicon-export"></span> 下载列表数据（批量导出）</button>
